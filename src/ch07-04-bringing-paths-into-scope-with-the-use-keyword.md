...
Listing 7-11: `use`를 사용하여 모듈을 스코프에 가져오기
...
Listing 7-12: `use` 문은 해당 스코프에서만 적용됩니다.
...
Listing 7-13: `use`를 사용하여 `add_to_waitlist` 함수를 스코프에 가져오기(불 idiomatic)
...
Listing 7-14: `HashMap`을 전통적인 방식으로 범위에 가져오기
...
Listing 7-15: 동일한 이름을 가진 두 유형을 동일한 범위로 가져오려면 부모 모듈을 사용해야 합니다.
...
Listing 7-16: `use` 키워드로 유형을 가져올 때 `as` 키워드를 사용하여 유형을 새로 이름 지정하기
...
Listing 7-17: `pub use`를 사용하여 다른 코드에서 사용할 수 있는 이름 만들기
